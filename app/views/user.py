...
from ..misc import engine, send_email, is_user_admin
...
def view(user):
    ...
    user_is_admin = misc.is_user_admin(user.uid)
    ...
    return engine.get_template('user/profile.html').render(
        {'user': user, 'level': level, 'progress': progress, 'postCount': pcount, 'commentCount': ccount,
         'givenScore': givenScore, 'invitecodeinfo': invitecodeinfo, 'badges': badges, 'owns': owns, 'mods': mods, 'habits': habit, 'user_is_admin': user_is_admin,
